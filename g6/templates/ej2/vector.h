...
template <class T> class vector {
    ...
    private:
        ...
        //Sólo copia los valores y el puntero _elem (NO COPIA LOS DATOS)
        void swap_internals(vector<T>& v2);
...
template <class T> void vector<T>::swap_internals(vector<T>& v2){
    _elem = v2._elem;
    _size = v2._size;
    _stored = v2._stored;
}
...
// REVISAR ESTA FUNCION Y SWAP_INTERNALS. Hay una forma mejor de hacer esto? (robado de StackOverflow: https://stackoverflow.com/questions/3279543/what-is-the-copy-and-swap-idiom)
template <class T> void vector<T>::operator = (vector<T>& origen){
    vector<T>* aux = new vector(origen);
    swap_internals(*aux);
}
